...
class Webrtc extends EventTarget {
    ...
    _handleOnTrack(socketId, event) {
        if (event.streams && event.streams[0]) {
            ...
            // Always emit the stream event first with the actual media stream
            this.log('Emitting stream event for', socketId);
            this._emit('stream', { socketId, stream: event.streams[0] });
            
            // If this is a new user and we haven't emitted the newUser event yet,
            // emit it separately WITHOUT the stream to ensure proper placeholder handling
            if (!this.pcs[socketId].hasEmittedNewUser) {
                this.log('Emitting newUser event for', socketId);
                // Important: Don't pass the stream here, let the stream event handle that
                this._emit('newUser', { socketId });
                this.pcs[socketId].hasEmittedNewUser = true;
            }
            
            this.log('Remote stream added for', socketId);
        } else {
            this.warn('No streams in track event for', socketId);
            
            // If we don't have a stream but this is a new user, still emit the newUser event
            if (!this.pcs[socketId].hasEmittedNewUser) {
                this.log('Emitting newUser event without stream for', socketId);
                this._emit('newUser', { socketId });
                this.pcs[socketId].hasEmittedNewUser = true;
            }
        }
