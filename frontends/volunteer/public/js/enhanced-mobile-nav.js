...
class EnhancedMobileNav {
    ...
    init() {
        // Don't initialize enhanced nav if we're on an authenticated page (nav-loader handles this)
        // Check if nav-container exists and has content (loaded by nav-loader)
        const navContainer = document.getElementById('nav-container');
        if (navContainer && navContainer.innerHTML.trim() !== '') {
            console.log('Enhanced mobile nav: Skipping initialization - authenticated page detected');
            return;
        }
        
        // Only initialize if we have the hamburger button (unauthenticated pages)
        const hamburgerButton = document.getElementById('mobile-menu-toggle');
        if (!hamburgerButton) {
            console.log('Enhanced mobile nav: Skipping initialization - no hamburger button found');
            return;
        }
