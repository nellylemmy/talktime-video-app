...
/* TalkTime auth shim: injects the stored JWT into every io() handshake (socket.handshake.auth.token).
   Appended here so it always runs immediately after the library loads, before any io() call. */
(function () {
    if (typeof window === 'undefined' || !window.io) return;
    var orig = window.io;
    function getToken() {
        try {
            var roles = ['volunteer', 'student', 'admin'];
            for (var i = 0; i < roles.length; i++) {
                var t = localStorage.getItem(roles[i] + '_talktime_access_token');
                if (t) return t;
            }
    ...
    function wrapped() {
        ...
        var url, opts;
        if (typeof args[0] === 'string') { url = args[0]; opts = args[1] || {}; }
        else { opts = args[0] || {}; }
        var token = getToken();
        if (token) {
            opts.auth = opts.auth || {};
        ...
        return url !== undefined ? orig(url, opts) : orig(opts);
    }
