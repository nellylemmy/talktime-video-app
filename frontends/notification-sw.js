...
// Handle sound requests from main thread
...
async function handlePushMessage(data) {
    ...
    const { title, body, icon, badge, tag, actions, url, sound_type, priority } = data;
    
    // Play sound if supported and requested
    if (sound_type) {
        try {
            // Trigger sound in main thread via postMessage
            const clients = await self.clients.matchAll();
            clients.forEach(client => {
                client.postMessage({
            ...
            console.error('Error triggering push notification sound:', error);
        }
    }
...
function handleNotificationSound(data) {
    // Service worker sound handling - limited capabilities
    // Main sound handling should be done in main thread
    console.log('🔊 Service worker received sound request:', data);
    
    // Forward to main thread
    self.clients.matchAll().then(clients => {
        clients.forEach(client => {
            client.postMessage({
                type: 'notification-sound-request',
                data: data
            });
        });
    });
